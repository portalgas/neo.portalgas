<template>
  <div class="box">
    ...
    >
      ...
      <span class="item-prezzo">{{ cart.article.prezzo }} &euro;</span>
      <span class="item-prezzo">{{ cart.qty * cart.article.prezzo }} &euro;</span>
    </div>
  ...
  methods: {
    ...
    totalPrice() {
      return this.getArticleInCart.reduce(
        (current, next) => current + (next.qty * next.article.prezzo),
        0
      );
    },
    showPopupCart() {
