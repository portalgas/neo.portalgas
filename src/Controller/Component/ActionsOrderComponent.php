...
class ActionsOrderComponent extends CartSuperComponent {
	...
	*/
		...
		foreach ($templatesOrdersStatesOrdersActions as $numResult => $templatesOrdersStatesOrdersAction) {
			...
			*/
			...
			if($orderActionOk) {
				...
				$orderActions[$i]['url'] = $this->drawjLink($orders_action->controller, $orders_action->action, ['delivery_id' => $order->delivery_id, 'order_id' => $order->id]);
				...
				if(!empty($orders_action->query_string)) {
					...
					switch ($orders_action->query_string) {
						case 'FilterArticleOrderId':
							...
							$orderActions[$i]['url'] = $this->drawjLink($orders_action->controller, $orders_action->action, ['delivery_id' => $order->delivery_id, 'order_id' => $order->id, 'FilterArticleOrderId' => $order->id]);
							break;
					}
				}
				$i++;
	...
	/*
	 * controllo che l'utente appartenga al gruppo GasGroups
	 */
