...
echo '<div class="row">';
echo '<div class="col-md-12">';
echo $this->Form->control('nota_evidenza', ['label' => 'Maggior informazioni sulla consegna', 'options' => $nota_evidenzas, 'value' => $gasGroupDelivery->delivery->nota_evidenza, 'v-model' => 'nota_evidenza_selected']);
echo '</div>';
echo '</div>'; // row

echo '<div class="row" v-if="is_nota_evidenza">';
echo '<div class="col-md-12">';
echo $this->Form->control('nota', ['type' => 'textarea', 'v-model' => 'nota_evidenza', 'value' => $gasGroupDelivery->delivery->nota]);
