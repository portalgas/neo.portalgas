      ...
      <div class="box box-solid">
        ...
        <div class="box-body">
          <dl class="dl-horizontal">
            ...
            <dd><?= h($mappingValueType->code) ?></dd>
            <dt scope="row"><?= __('Name') ?></dt>
            <dd><?= h($mappingValueType->name) ?></dd>
            <dt scope="row"><?= __('Id') ?></dt>
            <dd><?= $this->Number->format($mappingValueType->id) ?></dd>
            <dt scope="row"><?= __('Sort') ?></dt>
            <dd><?= $this->Number->format($mappingValueType->sort) ?></dd>
            <dt scope="row"><?= __('Created') ?></dt>
            <dd><?= h($mappingValueType->created) ?></dd>
            <dt scope="row"><?= __('Modified') ?></dt>
            <dd><?= h($mappingValueType->modified) ?></dd>
            <dt scope="row"><?= __('Is System') ?></dt>
            <dd><?= $mappingValueType->is_system ? __('Yes') : __('No'); ?></dd>
            <dt scope="row"><?= __('Is Active') ?></dt>
            <dd><?= $mappingValueType->is_active ? __('Yes') : __('No'); ?></dd>
          </dl>
        </div>
      </div>
    </div>
    ...
    <div class="col-md-12">
      <div class="box box-solid">
        <div class="box-header with-border">
          ...
          <h3 class="box-title"><?= __('Match') ?></h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
            <?= $this->Text->autoParagraph($mappingValueType->match); ?>
        </div>
      </div>
    </div>
  </div>
